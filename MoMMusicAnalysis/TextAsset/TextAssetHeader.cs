...
namespace MoMMusicAnalysis
{
    public class TextAssetHeader
    {
        ...
        public List<byte> UnityId { get; set; } // 0x26? bytes in length
        public int DuplicateDataSize { get; set; }
        ...
        public int EmptyDataAfterSectionLength { get; set; }
        ...
        public TextAssetHeader ProcessHeader(FileStream musicReader)
        {
            // Get Unity Version
            ...
            // Get Unity Version
            this.UnityId = musicReader.ReadBytesFromFileStream(0x25);

            // Get Header Size (Duplicate Portion Size)
            reversedData = musicReader.ReadBytesFromFileStream(4);
            reversedData.Reverse();
            this.DuplicateDataSize = BitConverter.ToInt32(reversedData.ToArray());
            ...
            // Get Duplicate Data
            this.DuplicateData = musicReader.ReadBytesFromFileStream(0x2C2B); // ACTUALLY - 2CBD is the length we skip above

            // Get Section Length
            reversedData = musicReader.ReadBytesFromFileStream(4);
            reversedData.Reverse();
            this.SectionLength = BitConverter.ToInt32(reversedData.ToArray());

            // Get Empty Data After Section Length
            this.EmptyDataAfterSectionLength = BitConverter.ToInt32(musicReader.ReadBytesFromFileStream(4).ToArray());

            this.Sections = new Section[this.SectionLength];
